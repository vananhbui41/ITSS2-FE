                    ...
                    <Grid item xs={12} >
                        <FormControl fullWidth>
                            <Autocomplete
                                ...
                                id="combo-box-demo"
                                options={words}
                                renderInput={(params) => <TextField {...params} label="Word" onChange={setWord(params.inputProps.value)} />}
                            />
                        </FormControl>
                    </Grid>
                    <Grid item xs={6} >
                        <FormControl fullWidth>
                            ...
                            <Select
                                ...
                                label="Context"
                                multiple
                                onChange={(e) => setContext(typeof e.target.value === 'string' ? e.target.value.split(',') : e.target.value)}
                                input={<OutlinedInput />}
                            ...
                            >
                                {categories.filter(item => item.category_id === 1).map(item => (
                                    <MenuItem key={item.id} value={item.name} style={getStyles(item.name, categories, theme)}>{ item.name} </MenuItem>
                                ))}
                            </Select>
                        </FormControl>
                    </Grid>
                    ...
                    <Grid item xs={12} >
                        <FormControl fullWidth>
                            ...
                            <Select
                                ...
                                value={topic}
                                label="Topic"
                                onChange={(e) => setTopic(typeof e.target.value === 'string' ? e.target.value.split(',') : e.target.value)}
                                input={<OutlinedInput />}
                                renderValue={(selected) => (
                                    <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 0.5 }}>
                                        {selected.map((value) => (
